...
Initialize a study through the validated Bayescycle CLI rather than copying or
editing canonical JSON by hand:

```bash
uv run --package bayescycle bayescycle study init <study> \
...
Canonical state mutation must go through Bayescycle. Initialize only when
explicitly requested. Apply a patch only after explicit human approval:

```bash
uv run --package bayescycle bayescycle study apply \
