...
that authored them. Its keys are non-empty strings. It is provenance only:
consumers never evaluate it, require it to match design variables, or use it
for generation. Exact `design.json` bytes remain authoritative. Plans authored
through direct JSON omit the field.
