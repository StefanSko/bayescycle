"""IR serialization for resolved model metadata (``bayeswire_ir`` version 1).
...
``spec/ir-format-v1.md`` and the generated ``spec/ir-v1-tags.md``.
"""
...
def _encode_value(value: object) -> JsonValue:
    ...
    if hasattr(value, "log_prob"):
        raise UnserializableDistribution(
            ...
            f"bayeswire.ir.register_distribution({type(value).__name__}), "
...
def meta_from_dict(document: object) -> ModelMeta:
    ...
    if not isinstance(document, dict):
        raise MalformedIRDocument(
            ...
            "Pass the parsed top-level object of a bayeswire_ir file."
        )
    ...
    if version != IR_VERSION:
        raise UnsupportedIRVersion(
            ...
            "meta_to_dict(...) or use a bayeswire build that reads this version."
        )
    if _MODEL_KEY not in envelope:
        raise MalformedIRDocument(
            f"IR documents must contain a {_MODEL_KEY!r} key holding the encoded model."
        )
    extra_fields = sorted(set(envelope) - {_VERSION_KEY, _MODEL_KEY})
    if extra_fields:
        raise MalformedIRDocument(
            f"IR envelopes must contain exactly the keys {_VERSION_KEY!r} and "
            f"{_MODEL_KEY!r}; remove the unexpected fields {extra_fields}."
        )
...
def _decode_node(value: object) -> object:
    ...
    if spec is None:
        raise UnknownNodeTag(
            ...
            "(bayeswire.ir.register_distribution(...)) before decoding, or restrict the "
            "document to the built-in tags listed in spec/ir-v1-tags.md."
        )
...
def render_ir_v1_tag_spec() -> str:
    """Render the generated built-in tag and field spec for ``spec/ir-v1-tags.md``."""
    lines = [
