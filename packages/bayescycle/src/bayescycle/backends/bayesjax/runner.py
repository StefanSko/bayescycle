"""In-process bayesjax sampling backend."""
...
def run_bayesjax_sample(command: BayesjaxSampleCommand) -> int:
    """Run bayesjax sampling in this Python process and write posterior.ndjson."""
    ...
    try:
        ...
        raise InProcessBackendError(
            "bayesjax in-process backend requires JAX and BlackJAX. "
...
def run_bayesjax_prior_predictive(command: BayesjaxPriorPredictiveCommand) -> int:
    """Run bayesjax prior-predictive simulation and write prior_predictive.ndjson."""
    ...
    try:
        ...
        raise InProcessBackendError(
            "bayesjax in-process backend requires JAX. "
            "Install with `bayescycle[inproc]` or use `--backend bayesite`."
        ) from exc
    data = _load_data(command.data_path.path)
    try:
...
run_in_process_sample = run_bayesjax_sample
...
def _load_bind_function() -> _BindModelFunction:
    try:
        ...
        raise InProcessBackendError(
            "bayesjax in-process backend requires the bayesjax package. "
            "Install with `bayescycle[inproc]` or use `--backend bayesite`."
        ) from exc
