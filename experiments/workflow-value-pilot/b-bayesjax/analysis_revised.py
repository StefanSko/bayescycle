"""Run once: uv run --project .. python analysis_revised.py (from this directory)."""
import hashlib
...
from model_revised import ClinicModel
...
def fit(label, input_name, seed, provenance):
    ...
    predictive_seed = 4202 if label == 'revised' else 4302
    ...
    out = dict(beta=beta, divergences=int(native['sampling_is_divergent'].sum()),
               ...
               paths=dict(samples=f'results/{label}/posterior.npz', figures=[f'results/{label}/trace.png',f'results/{label}/posterior_predictive.png'], native_diagnostics=f'results/{label}/native_diagnostics.npz', prior_predictive='results/revised-prior/prior_predictive.png'),
               provenance=provenance)
    if label == 'revised-recovery':
    ...
    (folder / 'COMPLETE').write_text('Revised-prior computation completed; awaits human review. See STUDY.md.\n')
...
def main():
    ...
    (ROOT / 'results/revised-model-ir.json').write_bytes(wire)
    files = ['model_revised.py','analysis_revised.py','input/data.json','input/recovery.json','input/recovery-truth.json','results/revised-model-ir.json','revision-preservation.json']
    ...
    save_json(ROOT / 'results/revised-provenance.json', provenance)
    prior_dir = ROOT / 'results/revised-prior'; prior_dir.mkdir()
    ...
    fit('revised-recovery','input/recovery.json',4301,provenance)
    fit('revised','input/data.json',4201,provenance)
    save_json(ROOT / 'results/revised-execution.json', dict(total_script_wall_seconds=time.perf_counter()-started, order=['revised-prior','revised-recovery','revised'], fit_attempts_this_invocation=2, prior_failed_invocations=0, recovery_same_seed_repair_retries=0))
