...
def fit(label, input_name, seed, provenance):
    ...
    for phase in ['warmup', 'sampling']:
        for f in dataclasses.fields(getattr(result.diagnostics, phase)):
            native[phase + '_' + f.name] = np.asarray(getattr(getattr(result.diagnostics, phase), f.name))
    ...
    (folder / 'COMPLETE').write_text('Completed; no fit retries.\n')
...
def main():
    ...
    save_json(ROOT / 'results/execution.json', dict(total_script_wall_seconds=time.perf_counter()-started, order=['prior','recovery','initial'], fit_attempts=2, retries=0))
