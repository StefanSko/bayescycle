...
Commands: uv run --project .. python analysis.py prior|recovery|initial
...
def fit(stage):
    ...
    result = {'beta': summaries['beta'], 'parameters': summaries,
        ...
        'sha256': {str(p):sha(p) for p in [Path('model.py'), original, run/'model.ir.json', run/'data.json']}}
    if stage=='recovery':
    ...
    plt.close('all')
    result['figures'] = [str(run/'trace.png')] + ([str(run/'posterior_predictive.png')] if stage=='initial' else [])
...
if __name__ == '__main__':
    ...
    elif stage in ('initial','recovery'): fit(stage)
