"""Run once: uv run --project .. python analysis_revised.py
Refuses existing revised outputs; verifies initial artifact preservation.
"""
import os
...
from model_revised import model
...
def fit(data, stage, seed, truth=None):
    ...
    result = dict(beta=beta_summary, divergences=divergences,
                  ...
                  provenance='results/revision-provenance.json')
    ...
    if stage == 'revised':
        ...
        result['paths'].update(posterior_predictive='results/revised/posterior_predictive.npz',
                               posterior_predictive_figure='results/revised/posterior_predictive.png',
                               prior_predictive_figure='results/revised-prior/prior_predictive.png')
...
def main():
    ...
    for name in ['revised', 'revised-prior', 'revised-recovery', 'revision-provenance.json',
                 'revision-execution.json', 'REVISION_COMPLETE']:
        assert not (results / name).exists(), f'Refusing existing output: {name}'
    original = json.loads((results / 'provenance.json').read_bytes())
    for name, expected in original['sha256'].items():
        assert hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == expected, name
    preserved_paths = [ROOT / 'model.py', ROOT / 'analysis.py'] + sorted(p for p in results.rglob('*') if p.is_file())
    preserved = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest() for p in preserved_paths}
    files = ['model_revised.py', 'analysis_revised.py', 'model.py', 'analysis.py',
             'input/data.json', 'input/recovery.json', 'input/recovery-truth.json',
             '../REVISION-BRIEF.md', '../PROTOCOL.md']
    save_json(results / 'revision-provenance.json', dict(
        preserved_initial_sha256=preserved,
        change='Only beta prior SD: 1 -> 0.25; initial source and artifacts retained',
        ...
        command='uv run --project .. python analysis_revised.py', model_stage='revised',
        status='Engineering fixture; awaits human scientific review'))
    data = load('data.json')
    prior_dir = results / 'revised-prior'
    prior_dir.mkdir()
    ...
    save_json(prior_dir / 'result.json', dict(seed=4200, draws=200, provenance='results/revision-provenance.json'))
    ...
    fit(load('recovery.json'), 'revised-recovery', 4301, json.loads((ROOT / 'input/recovery-truth.json').read_bytes()))
    fit(data, 'revised', 4201)
    for name, expected in preserved.items():
        assert hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == expected, name
    save_json(results / 'revision-execution.json', dict(total_wall_seconds=time.perf_counter() - start,
                                             stage_order=['revised-prior', 'revised-recovery', 'revised'], fit_attempts=2,
                                             retries=0, initial_preservation_verified=True))
    (results / 'REVISION_COMPLETE').write_text('Scripted revision complete; awaits human review.\n')
